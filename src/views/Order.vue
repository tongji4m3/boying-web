    ...
    <div>
      ...
      <div class="table-container">
        ...
        >
          ...
          <el-table-column label="演出名称" prop="name" align="center">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async handleDelete(index, row) {
      try {
        const res = await axios.post(this.$api.deleteUserOrder + "/" + row.id);
        console.log("删除订单");
        console.log(res);
    ...
    async handleRefund(index, row) {
      try {
        const res = await axios.post(this.$api.refundUserOrder + "/" + row.id);
        console.log("取消订单");
        console.log(res);
        if (res.data.code === 200) {
