...
export default {
  ...
  created() {
    if (this.$route.query != null) this.rcategory = this.$route.query.id;
    ...
    async getShow() {
      ...
      if (result.data.code === 200) {
        ...
        for (let i = 0; i < this.showList.length; i++) {
          this.showList[i].startTime = this.showList[i].startTime.substring(
            0,
            19
          );
          this.showList[i].startTime = this.showList[i].startTime.replace(
            "T",
            " "
          );
        ...
        this.showList = [];
        this.totalCount = 0;
        this.$message.error(result.data.code + result.data.message);
      }
